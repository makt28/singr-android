...
3. 打开 App,填 API 地址、Key、Node ID,选节点类型(AnyTLS 或 Hysteria2),再从文件管理器选证书和私钥两个文件,点保存。
4. 点启动节点。之后到系统设置里开启 **Always-on VPN + Lockdown**,并把 App 加进电池优化白名单,这样它被杀掉或重启后会自己起来。
